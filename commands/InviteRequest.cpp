...
/*   Updated: 2024/03/12 13:46:18 by abait-ta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    InviteMessage(std::string& clientMsg, int __fd)
{
    ...
    if (OccurSpace == 0 || OccurSpace == 1){
        std::string response = ":" + MYhost::GetHost() + " 461 " + Server::ServerClients.at(__fd).nickname + " INVITE " + ": NO ENOUGH PARAMETERS" + "\n" +
            NumericReplies(MYhost::GetHost(), "999", Server::ServerClients.at(__fd).nickname, "INVITE", "<USER> && <CHANNEL>");
                send(__fd, response.c_str(), response.length(), 0);
    }
    else
    {
