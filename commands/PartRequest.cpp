...
/*   Updated: 2024/03/12 13:49:59 by abait-ta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    PartProcessor(DEQUE& Channels, int __fd, std::string& Reason)
{
    ...
    while (++i < Channels.size())
    {
        ...
        {
            if ((*Finder).IsalreadyMember(Server::ServerClients.at(__fd).nickname) == true)
            {
                ...
                if ((*Finder).IsMediator(Server::ServerClients.at(__fd).nickname) == true){
                    ...
                    (*Finder).PartMediator(Server::ServerClients.at(__fd).nickname);
                }
                ...
                if ((*Finder).Roomsize() == 0 || (*Finder)._Mediators.size() == 0){// if no admin or no member the Channel Will be Destroyed
                    ...
                    if ((*Finder)._Mediators.size() == 0){
                        ...
                        std::string resp = ":" + MYhost::GetHost() + " 999 " + (*Finder)._RoomName + " Destroyed at : " + ctime(&theTime);
